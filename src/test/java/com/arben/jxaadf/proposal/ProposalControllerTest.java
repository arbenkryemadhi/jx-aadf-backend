...
public class ProposalControllerTest {
    ...
    void createProposal_ShouldReturnSuccess() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(post("/api/proposal/create").param("tenderId", "1")
                .param("authorId", "test@example.com").param("title", "Test Proposal")
                .param("description", "Proposal Description").param("price", "5000 EUR")
                .param("status", "Pending").param("createdDate", "2025-05-01"))
    ...
    @Test
    void updateProposal_ShouldReturnSuccess() throws Exception {
        // Arrange
        when(proposalRepository.updateProposal(any(Proposal.class)))
                .thenReturn("Proposal updated successfully");

        // Act & Assert
        mockMvc.perform(put("/api/proposal/update").param("proposalId", "1").param("tenderId", "1")
                .param("authorId", "test@example.com").param("title", "Updated Proposal")
                .param("description", "Updated Description").param("price", "6000 EUR")
                .param("status", "Accepted")).andExpect(status().isOk())
    ...
    void getAllUserProposals_ShouldReturnList() throws Exception {
        ...
        mockMvc.perform(get("/api/proposal/getalluser").param("userId", "test@example.com"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].proposalId").value(testProposal.getProposalId()))
    ...
    void getProposalById_ShouldReturnProposal() throws Exception {
        ...
        mockMvc.perform(get("/api/proposal/getbyid").param("proposalId", "1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.proposalId").value(testProposal.getProposalId()))
                .andExpect(jsonPath("$.title").value(testProposal.getTitle()))
                .andExpect(jsonPath("$.description").value(testProposal.getDescription()));
    ...
    void getAllUserProposals_WithInvalidUserId_ShouldReturnEmptyList() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/proposal/getalluser").param("userId", "nonexistent@example.com"))
