...
public class ProposalReviewControllerTest {
    ...
    void createProposalReview_ShouldReturnSuccess() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(post("/api/proposalreview/create").contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(testProposalReview)))
                .andExpect(status().isOk())
                .andExpect(content().string("Proposal review created successfully"));

        verify(proposalReviewRepository, times(1)).createProposalReview(any(ProposalReview.class));
    }
    ...
    void updateProposalReview_ShouldReturnSuccess() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(put("/api/proposalreview/update").contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(testProposalReview)))
                .andExpect(status().isOk())
                .andExpect(content().string("Proposal review updated successfully"));

        verify(proposalReviewRepository, times(1)).updateProposalReview(any(ProposalReview.class));
    }
    ...
    void deleteProposalReview_ShouldDeleteProposalReview() throws Exception {
        ...
        mockMvc.perform(delete("/api/proposalreview/delete").param("proposalReviewId", "1"))
                .andExpect(status().isOk())
                .andExpect(content().string("Proposal review deleted successfully"));
    ...
    void getProposalReviewById_ShouldReturnProposalReview() throws Exception {
        ...
        mockMvc.perform(get("/api/proposalreview/getreview").param("proposalReviewId", "1"))
                ...
                .andExpect(jsonPath("$.description").value(testProposalReview.getDescription()));
    ...
    void getAllProposalReviewsOfProposal_ShouldReturnList() throws Exception {
        ...
        mockMvc.perform(get("/api/proposalreview/getbyproposalid").param("proposalId", "2"))
                ...
                .andExpect(jsonPath("$[0].proposalReviewId")
                        .value(testProposalReview.getProposalReviewId()))
                .andExpect(jsonPath("$[0].title").value(testProposalReview.getTitle()));
    ...
    void getAllProposalReviewsOfProposal_WithInvalidProposalId_ShouldReturnEmptyList()
            throws Exception {
        // Arrange
        when(proposalReviewRepository.getAllProposalReviewsOfProposal(999)).thenReturn(List.of());

        // Act & Assert
        mockMvc.perform(get("/api/proposalreview/getbyproposalid").param("proposalId", "999"))
    ...
    @Test
    void getAllProposalReviewsOfUser_WithInvalidUserId_ShouldReturnEmptyList() throws Exception {
        // Arrange
        when(proposalReviewRepository.getAllProposalReviewsOfUser("nonexistent"))
                .thenReturn(List.of());

        // Act & Assert
        mockMvc.perform(get("/api/proposalreview/getbyuserid").param("userId", "nonexistent"))
                .andExpect(status().isOk()).andExpect(jsonPath("$").isArray())
                .andExpect(jsonPath("$").isEmpty());

        verify(proposalReviewRepository, times(1)).getAllProposalReviewsOfUser("nonexistent");
    }
}
